...
class WidowxROS(gym.Env):
    def __init__(self, episode_len=100):
        ...
        self.max_iter = episode_len
        max_action = 100*(math.pi/16.0)/self.max_iter

        # last 3 are gathered from data, more precision needed there
        bounds = np.array([2.617, 1.571, 1.571, 1.745, 0.37, 0.37, 0.51])
        # bounds = np.array([2.617, 1.571, 1.571, 1.745, 0.45, 0.45, 0.58])
        self.action_space = spaces.Box(-max_action, max_action, shape=(4,))
    ...
    def step(self, action, save=True):
        ...
        if save:
            ...
            if not collision:
                # tmp_position = get_end_effector_pos(self.moveit_fk, self.rs)
